...
<div class="min-h-screen bg-dashboard-grid px-4 py-12 text-ink">
  <div class="mx-auto grid min-h-[calc(100vh-6rem)] max-w-6xl items-center gap-8 lg:grid-cols-2">
    <section class="space-y-4">
      ...
      <h1 class="text-4xl font-semibold tracking-tight">Secure evidence collection for cases that matter.</h1>
      <p class="max-w-xl text-slate-600">Store evidence, extract text, build timelines, and export reports from a single protected workspace.</p>
    </section>
    ...
    <section class="panel p-8">
      ...
      <div class="mt-6 space-y-4">
        ...
        <button class="button-primary w-full" on:click={submitLogin}>Log in</button>
        <p class="text-sm text-slate-600">Need an account? <a class="font-semibold text-blue-600" href="/register">Register</a></p>
        {#if errorMessage}
